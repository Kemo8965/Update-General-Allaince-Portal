...
export const mutations = {
  ...
  [CREATE_POLICY](state, policy) {
    state.inactivePolicies.push(policy)
  },
...
export const actions = {
  ...
  async createPolicy({ state, commit, rootGetters }) {
    try {
      ...
      policy.createdBy = this.$auth.user.id.toString()
      policy.agencyID = this.$auth.agent.agenecyID.toString()

      const individualClient = rootGetters['clients/individualClients'].some(
        ({ id }) => id.toString() === policy.clientID.toString()
      )

      if (individualClient) {
        policy.clientType = 'INDIVIDUAL'
      } else {
        policy.clientType = 'CORPORATE'
      }

      await asyncForEach(policy.vehicles, async (vehicle, index, vehicles) => {
        const premiumResponse = await this.$axios.$post('/tp/getPremium', {
          numOfQuarters: policy.numOfQuarters,
          productTypeID: policy.productTypeID,
        })

        vehicle.premium = premiumResponse.data.premium

        if (index === vehicles.length - 1) {
          const response = await this.$axios.$post(`/tp/create`, policy)
